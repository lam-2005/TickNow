...
const TableTicket = ({
  ...
  return (
    <>
      ...
      <div className="w-full overflow-x-auto max-md:text-xs max-md:[&_th]:px-2 max-md:[&_th]:py-1 max-md:[&_td]:px-2 max-md:[&_td]:py-1">
        <table className="min-w-[700px] w-full text-sm sm:text-base">
          <thead className="[&_th]:text-center [&_th]:py-2 [&_th]:px-4 [&_th]:text-sm [&_th]:font-semibold border-1 border-primary">
            <tr className="bg-primary text-white">
              ...
              <th>Chi tiết vé</th>
              <th>Trạng thái</th>
              <th className="sm:table-cell">Đánh giá phim</th>
            </tr>
          </thead>
            ...
            <tbody className="[&_tr]:even:bg-background-card border-1 border-foreground">
              {newData?.tickets.map((item) => (
                <tr key={item._id} className="text-center">
                  ...
                  <td className="py-2 sm:table-cell">
                    {item?.status_cmt === 1 ? (
                      <p
              ...
              <tr className="bg-background! border-t-1 border-t-foreground">
                <td colSpan={6}>
                  <div className="flex items-center justify-end gap-5 p-5 py-3">
                    <div>
                      Hiển thị{" "}
                      {(page - 1) * (newData?.pagination.limit || 5) + 1} –{" "}
                      {Math.min(
